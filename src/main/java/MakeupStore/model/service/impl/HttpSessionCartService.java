package MakeupStore.model.service.impl;
...
import MakeupStore.model.dao.impl.JdbcStockDao;
...
public class HttpSessionCartService implements CartService {
    ...
    private HttpSessionCartService() {
        makeupDao = JdbcMakeupDao.getInstance();
    ...
    public void add(Cart cart, Long productId, int quantity, HttpSession currentSession) throws OutOfStockException, ServiceException {
        ...
        synchronized (currentSession) {
            Makeup phone;
            try {
                phone = makeupDao.get(productId).orElse(null);
     ...
     */
    private int countingQuantityIncludingCart(Cart cart, Makeup phone) throws ServiceException {
        int result = 0;
        try {
        ...
        Integer quantityInCart = cart.getItems().stream()
                .filter(currProduct -> currProduct.getMakeupItem().equals(phone))
    ...
    public void delete(Cart cart, Long productId, HttpSession currentSession) {
        synchronized (currentSession) {
            cart.getItems().removeIf(item -> productId.equals(item.getMakeupItem().getId()));
            reCalculateCart(cart);
        }
    }
     ...
     * @return cartItem
     */
    private Optional<CartItem> getCartItemMatch(Cart cart, Makeup product) {
        return cart.getItems().stream().
                filter(currProduct -> currProduct.getMakeupItem().getId().equals(product.getId())).
                findAny();
    }
    ...
    public void remove(HttpSession currentSession, Long phoneId) {
        Cart cart = getCart(currentSession);
        cart.getItems().removeIf(item -> phoneId.equals(item.getMakeupItem().getId()));
        reCalculateCart(cart);
    }
}
