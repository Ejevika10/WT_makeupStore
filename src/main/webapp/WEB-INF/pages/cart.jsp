...
<jsp:useBean id="cart" scope="session" type="MakeupStore.model.entities.cart.Cart"/>
            ...
            <c:if test="${cart.items.size() > 0}">
                    ...
                    <table class="table table-hover table-bordered text-center">
                        ...
                        <c:forEach var="cartItem" items="${cart.items}">
                            <tr>
                                <td>${cartItem.makeupItem.brand}</td>

                                <td>$ ${cartItem.makeupItem.price}</td>
                                <td>
                                    <input type="number" name="quantity" value="${cartItem.quantity}" min="1" required>
                                    <c:if test="${not empty inputErrors.get(cartItem.makeupItem.id)}">
                                        <div class="error" style="color: red">${inputErrors.get(cartItem.makeupItem.id)}</div>
                                    </c:if>
                                </td>
                                <td>
                                    <input type="hidden" name="id" value="${cartItem.makeupItem.id}">
                                    <button class="btn btn-danger" type="button" onclick="deleteCartItem(${cartItem.makeupItem.id})"><fmt:message key="button_delete" /></button>
                                </td>
                            </tr>
                        </c:forEach>
                    </table>
